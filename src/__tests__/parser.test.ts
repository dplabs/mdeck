...
describe("Parser", () => {
	...
	describe("macros", () => {
		it("expands macro to its return value", () => {
			const macros = {
				sum: (...args: string[]) =>
					String(args.reduce((acc, a) => acc + parseInt(a, 10), 0)),
			};
			expect(parser.parse("a ![:sum 1, 2, 3] b", macros)[0].content).toEqual([
				"a 6 b",
			]);
		});
		...
		it("expands macros recursively", () => {
			const macros = {
				upper: function (this: string | undefined) {
			...
			expect(parser.parse(".c1[.c2[x]]")[0].content).toEqual([
				{
					class: "c1",
					block: false,
					content: [{ class: "c2", block: false, content: ["x"] }],
				},
			]);
		});
	});
	...
	describe("slide properties", () => {
		...
		it("does not consume a word:value line that appears after other content", () => {
			// "Example: this line disappears" appears after real content — must stay as content.
			const slide = parser.parse(
		...
		it("extracts <style> block into properties.styles", () => {
			const src = "<style>\n  .red { color: red; }\n</style>\n\n# Slide 1";
			const slides = parser.parse(src);
			expect(slides[0].properties.styles).toContain(".red { color: red; }");
			expect(slides[0].content[0]).not.toContain("<style>");
			expect(slides[0].content[0]).toContain("# Slide 1");
		});
		...
		it("extracts multiple <style> blocks", () => {
			...
			const slides = parser.parse(src);
			expect(slides[0].properties.styles).toContain(".a { color: red; }");
			expect(slides[0].properties.styles).toContain(".b { color: blue; }");
		});

		it("handles interleaved properties and <style> blocks", () => {
			const src =
				"name: a\n<style>.red { color: red; }</style>\nclass: b\n\n# Content";
			const slides = parser.parse(src);
			expect(slides[0].properties.name).toBe("a");
			expect(slides[0].properties.styles).toContain(".red { color: red; }");
			expect(slides[0].properties.class).toBe("b");
		});

		it("extracts <style> block after properties", () => {
			const src =
				"class: center\n<style>.red { color: red; }</style>\n\n# Slide 1";
			const slides = parser.parse(src);
			expect(slides[0].properties.class).toBe("center");
			expect(slides[0].properties.styles).toContain(".red { color: red; }");
		});

		it("extracts <style> block after properties and blank lines", () => {
			const src =
				"class: center\n\n<style>.red { color: red; }</style>\n\n# Slide 1";
			const slides = parser.parse(src);
			expect(slides[0].properties.class).toBe("center");
		...
		it("stops at blank line if more front-matter follows (issue #4)", () => {
			const src =
				"class: center\n\n# Slide 1\n\n<style>this is content</style>";
			const slides = parser.parse(src);
			expect(slides[0].properties.class).toBe("center");
			expect(slides[0].properties.styles).toBeUndefined();
			expect(slides[0].content.join("")).toContain("# Slide 1");
			expect(slides[0].content.join("")).toContain(
				"<style>this is content</style>",
			);
		});
	});
